...
- `Samples~/UniTaskExtensionExamples/UniTaskExtension.Examples.asmdef`는 샘플 코드가 UniTask 예제를 직접 사용하므로 UniTask asmdef GUID로 보이는 `GUID:f51ebe6a0ceec4240a699833d6309b23`을 참조합니다. `Samples~`는 기본 설치 시 컴파일되지 않으며, Package Manager에서 샘플을 가져온 경우에만 프로젝트에 포함됩니다.
...
| Samples | `CubicEngine.UnitaskExtension.Samples` |
...
Samples~/
  UniTaskExtensionExamples/
  Runtime/
  UniTaskExtension.Examples.asmdef
```
